...
export default function TeamManagementPage() {
  ...
  if (loading) {
    return <div className="p-12 text-center text-outline animate-pulse">Loading teams...</div>
  }
          ...
          <div className="bg-surface-container-lowest w-full max-w-md rounded-[2.5rem] whisper-shadow overflow-hidden animate-fade-in" onClick={e => e.stopPropagation()}>
            <div className="p-8">
              ...
              <div className="bg-surface-container-low p-6 rounded-2xl border border-outline-variant/10 mb-6 space-y-4">
                ...
                <div>
                  <label className="text-[10px] uppercase tracking-widest font-bold text-outline-variant block mb-1">Login Email</label>
                  <p className="text-sm font-semibold text-on-surface">{approvalModal.email || approvalModal.mobile}</p>
                </div>
                <div>
                  <label className="text-[10px] uppercase tracking-widest font-bold text-outline-variant block mb-1">Status</label>
                  <p className="text-sm font-mono font-bold text-on-surface bg-white px-3 py-1 inline-block rounded shadow-sm">Ready for Login</p>
                </div>
              </div>
              <div className="flex flex-col gap-3">
                ...
                <button onClick={() => navigator.clipboard?.writeText(`Email: ${approvalModal.email}\nMobile: ${approvalModal.mobile}`)} className="w-full bg-surface-container-high text-on-surface-variant py-4 rounded-xl font-bold flex items-center justify-center gap-3 hover:bg-surface-variant transition-all">
